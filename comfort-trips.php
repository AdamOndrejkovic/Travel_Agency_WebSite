    ...
    <?php 
      $conn = false;
          ...
          if(file_exists($file)) {include($file);}
             ...
             $res = $conn->query('SELECT * FROM tripsmodel Where type = "Comfort" LIMIT 6');
             while($row=$res->fetch()) { 
             ...
             ?>
    <!-- Button to load more trips -->
    <script>
        $(document).ready(function() {
            var tripsCount = 6;
            $("button").click(function() {
                tripsCount = tripsCount + 6;
                $(".all-trips-box").load("load-ctrips.php", {
